...
<template>
  <PullRefresh class="circles-page thin-scroll" @refresh="fetchCircles">
    ...
    <template v-else>
      ...
      <section class="section" v-if="myCircles.length">
        ...
        <AnimateOnScroll animation="fadeUp" :stagger="80">
          <div class="card-row">
      ...
      <section class="section">
        ...
        <AnimateOnScroll v-if="circles.length" animation="fadeUp" :stagger="80">
          <div class="card-row">
...
.hero-title { font-size: 38px; font-weight: 900; margin: 0 0 8px; letter-spacing: -0.5px; color: var(--text-primary); }
.hero-sub { font-size: 16px; color: var(--text-secondary); margin: 0 0 20px; }
...
.section-title { font-size: 20px; font-weight: 800; margin: 0; color: var(--text-primary); letter-spacing: 1px; }
...
/* ── Cards ── */
.card-row { display: grid; grid-template-columns: repeat(auto-fill, minmax(172px, 1fr)); gap: 16px; }
.circle-card {
