...
import { ensureVerificationContracts } from './verification.service.js';
...
export async function listOfferings(params: ListOfferingsParams = {}) {
  ...
  return rows.map((o) => ({
    ...
    endpoint: { healthStatus: o.endpoint?.healthStatus ?? null },
  }));
}
...
export async function retrieveOffering(id: string, isAdmin: boolean) {
  const o = await prisma.loopOffering.findUnique({
    where: { id },
    include: {
      endpoint: { select: { healthStatus: true, hostMode: true } },
      definition: { select: { loopKind: true, code: true, name: true, description: true } },
    },
  });
  if (!o) return null;
  ...
  if (isAdmin) return { ...base, internalSuccessRate: o.internalSuccessRate };
  return base;
}
...
export async function runOffering(
  ...
  const o = await prisma.loopOffering.findUnique({
    where: { id: offeringId },
    include: {
      definition: { select: { code: true, loopKind: true, name: true } },
      endpoint: { select: { id: true, hostMode: true } },
    },
  });
