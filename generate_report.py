...
def add_form_field(doc, label, value=""):
    """添加表单格式的字段"""
    p = doc.add_paragraph()
    run = p.add_run(label)
    run.bold = True
...
p = doc.add_paragraph('2025-2026学年第二学期')
p.alignment = WD_ALIGN_PARAGRAPH.CENTER

p = doc.add_paragraph('《软件测试与质量控制》')
p.alignment = WD_ALIGN_PARAGRAPH.CENTER

p = doc.add_paragraph('课程实验报告')
p.alignment = WD_ALIGN_PARAGRAPH.CENTER
...
doc.add_paragraph('（一）硬件环境：', style='List Bullet')
hw_items = [
    'PC机一台',
    '内存要求：不少于2G',
    '磁盘空间要求：不少于50G剩余磁盘空间'
...
doc.add_paragraph('（二）软件环境：', style='List Bullet')
sw_items = [
...
doc.add_paragraph('（1）软件环境', style='List Bullet')
...
doc.add_paragraph('（2）硬件环境', style='List Bullet')
...
doc.add_paragraph('（3）测试用例统计', style='List Bullet')
...
doc.add_paragraph()
stat_table2 = doc.add_table(rows=2, cols=4)
stat_table2.style = 'Table Grid'
stat_table2.rows[0].cells[0].text = '总用例数'
stat_table2.rows[0].cells[1].text = '通过数'
stat_table2.rows[0].cells[2].text = '未通过数'
stat_table2.rows[0].cells[3].text = '通过率'
stat_table2.rows[1].cells[0].text = '16'
stat_table2.rows[1].cells[1].text = '15'
stat_table2.rows[1].cells[2].text = '1'
stat_table2.rows[1].cells[3].text = '93.75%'
...
doc.add_paragraph('缺陷描述：', style='List Bullet')
doc.add_paragraph('    当用户借阅图书后，系统中显示的已借阅数量未及时更新，导致用户可以超出借阅数量限制。', style='List Bullet')

doc.add_paragraph('复现步骤：', style='List Bullet')
doc.add_paragraph('    1. 使用账号登录系统', style='List Bullet')
doc.add_paragraph('    2. 当前已借阅4本书（达到限制的80%）', style='List Bullet')
doc.add_paragraph('    3. 借阅第5本书，提示"已达借阅上限"', style='List Bullet')
doc.add_paragraph('    4. 再次借阅第5本书，系统仍提示"已达借阅上限"', style='List Bullet')
doc.add_paragraph('    5. 刷新页面后，已借阅数量显示为5，但可以继续借阅', style='List Bullet')

doc.add_paragraph('预期结果：', style='List Bullet')
doc.add_paragraph('    每次借阅操作后，应实时更新已借阅数量，超出限制时立即拒绝借阅。', style='List Bullet')

doc.add_paragraph('实际结果：', style='List Bullet')
doc.add_paragraph('    借阅数量未实时更新，存在缓存导致判断延迟。', style='List Bullet')

doc.add_paragraph('缺陷修复建议：', style='List Bullet')
doc.add_paragraph('    1. 在借阅操作后立即刷新已借阅数量', style='List Bullet')
doc.add_paragraph('    2. 在前端和后端同时进行借阅数量校验', style='List Bullet')
doc.add_paragraph('    3. 减少前端缓存时间，或在关键操作时绕过缓存', style='List Bullet')

# 4.5 测试结果与建议
doc.add_heading('4.5 测试结果与建议', level=2)

doc.add_paragraph('测试结果：', style='List Bullet')
doc.add_paragraph('    本次功能测试共执行测试用例16个，通过15个，失败1个，通过率93.75%。', style='List Bullet')
doc.add_paragraph('    发现严重缺陷1个，位于图书借阅模块的借阅数量校验功能。', style='List Bullet')

doc.add_paragraph('改进建议：', style='List Bullet')
doc.add_paragraph('    1. 修复借阅数量实时更新问题，加强前后端双重验证', style='List Bullet')
doc.add_paragraph('    2. 增加并发测试，验证系统在高负载下的稳定性', style='List Bullet')
doc.add_paragraph('    3. 补充异常场景测试，如网络中断、服务器错误等', style='List Bullet')
doc.add_paragraph('    4. 建议增加自动化测试用例，提高回归测试效率', style='List Bullet')

# 五、被测系统与小组分工情况表
doc.add_heading('五、被测系统与小组分工情况表', level=1)

group_table = doc.add_table(rows=3, cols=4)
...
group_data = [
    ...
    ['', '测试模块', '开始时间', '结束时间'],
    ['', '用户登录', '2026-04-22', '2026-04-22']
]

# 简化表格
group_table.rows[1].cells[0].text = '图书管理系统'
group_table.rows[1].cells[1].text = 'V1.0'
group_table.rows[1].cells[2].text = '罗建明'
group_table.rows[1].cells[3].text = '独立完成全部功能测试'

# 六、实验小结
doc.add_heading('六、实验小结', level=1)

doc.add_paragraph('（一）问题和解决办法', style='List Bullet')
doc.add_paragraph('    问题1：测试过程中发现借阅数量统计存在延迟问题', style='List Bullet')
doc.add_paragraph('    解决办法：详细记录缺陷复现步骤，分析可能的原因（缓存、网络延迟等），并在缺陷报告中提出具体的修复建议。', style='List Bullet')

doc.add_paragraph('    问题2：测试用例设计时难以覆盖所有边界条件', style='List Bullet')
doc.add_paragraph('    解决办法：采用等价类划分和边界值分析相结合的方法，确保测试用例具有代表性和全面性。', style='List Bullet')

doc.add_paragraph('（二）心得体会', style='List Bullet')
doc.add_paragraph('    通过本次实验，我深入了解了功能测试的重要性及其在软件质量保障中的作用。以下是我的几点心得：', style='List Bullet')
doc.add_paragraph('    1. 功能测试是发现软件缺陷的重要手段，通过系统的测试可以发现许多隐藏的问题。', style='List Bullet')
doc.add_paragraph('    2. 黑盒测试方法（等价类划分、边界值分析）是设计测试用例的有效工具，可以提高测试效率。', style='List Bullet')
doc.add_paragraph('    3. 详细的缺陷报告对于开发人员修复问题非常重要，测试人员需要准确描述缺陷现象、复现步骤和预期结果。', style='List Bullet')
doc.add_paragraph('    4. 独立完成整个测试流程，让我对软件测试的完整生命周期有了更清晰的认识。', style='List Bullet')

doc.add_paragraph('（三）意见与建议', style='List Bullet')
doc.add_paragraph('    1. 建议在后续课程中增加自动化测试工具的实践内容，如Selenium、Appium等。', style='List Bullet')
doc.add_paragraph('    2. 建议增加性能测试和安全测试的实验内容，使测试知识体系更加完整。', style='List Bullet')
doc.add_paragraph('    3. 希望能有更多真实的项目案例供同学们实践，提高解决实际问题的能力。', style='List Bullet')

doc.add_paragraph()

# 签名栏
doc.add_paragraph('实验报告评分：')
doc.add_paragraph()
doc.add_paragraph('实验报告等级：')
doc.add_paragraph()
...
# 保存文档
output_path = Path(r'C:/Users/19617/Desktop/《软件测试与质量控制》实验报告（三）_已完成.docx')
