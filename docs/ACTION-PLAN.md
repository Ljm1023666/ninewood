...
3. 第 2 节是**三阶段路线**：按优先级排序，每项带验收标准。Codex 当前只执行**阶段 0**。
4. 第 3 节是**阶段 0 执行规格**：可直接据此落地，不需要再拆需求。
5. 第 4 节是**推进纪律**：基于仓库既有规则，避免返工。
6. 第 5 节是**风险登记**。

### Codex 执行边界

- **当前只做阶段 0**：补测试 + 删除无引用旧 service（如确认安全）。阶段 1/2 只作路线图，不在本轮实现。
- **测试策略**：阶段 0 沿用现有 Vitest + Prisma mock 风格，不强制接真实测试数据库，不新增 HTTP 集成测试框架。
- **禁止扩大范围**：不要实现 socket 广播、不要做 `autoAccept`、不要做公益拨付、不要做公开圈。
...
| Codex 误把路线图当实现规格 | 提前做阶段 1/2，扩大改动面 | 本文档明确当前只执行阶段 0；阶段 1 需另写 spec |
