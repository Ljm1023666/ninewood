...
async function joinByCode() {
  ...
  try {
    ...
    showJoinCode.value = false; joinCode.value = '';
    fetchAll();
  ...
  <div class="circle-detail">
    ...
    <div v-else-if="circle" class="content">
      ...
      <div class="action-strip">
        <button v-if="!isMember && circle.type === 'PUBLIC'" class="strip-btn primary" :disabled="joining" @click="joinCircle">
          {{ joining ? '加入中' : '加入圈子' }}
        </button>
        <button v-if="!isMember && circle.type === 'PRIVATE'" class="strip-btn primary" @click="showJoinCode = true">
          邀请码加入
        </button>
      ...
      <div v-if="showMembers && circle.members" class="members-panel">
        <div v-for="m in circle.members" :key="m.userId" class="member-row" @click="router.push('/profile/' + m.userId)">
          <div class="m-avatar">
            <img v-if="m.user?.avatarUrl" :src="m.user.avatarUrl" class="m-avatar-img" />
    ...
    <!-- Join code dialog -->
    <t-dialog v-model:visible="showJoinCode" header="邀请码加入" :footer="false">
      <div class="dialog-form">
        <t-input v-model="joinCode" placeholder="输入邀请码" @keyup.enter="joinByCode" />
        <t-button theme="primary" block :loading="joining" @click="joinByCode">加入</t-button>
      </div>
    </t-dialog>
  </div>
</template>
...
.circle-detail::-webkit-scrollbar { width: 4px; }
.circle-detail::-webkit-scrollbar-thumb { background: var(--border-color); border-radius: 2px; }
