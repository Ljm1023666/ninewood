  ...
  <div class="home-page" @scroll="onScroll">
    ...
    <div v-else class="demand-grid">
      ...
      <div v-for="(d, idx) in demands" :key="d.id" class="demand-card" :class="{ expanded: expandedId === d.id }" @click="toggleExpand(d.id)">
        <!-- Card glow border -->
        <div class="card-border" :style="{ background: `linear-gradient(135deg, ${certColor(d.user?.certificationLevel)}44, transparent)` }" />
        ...
        <div class="card-details" :class="{ open: expandedId === d.id }">
          ...
          <div class="card-head">
            <div class="publisher" @click.stop="router.push('/profile/' + d.user.id)">
              <div class="pub-avatar" :style="{ background: certColor(d.user?.certificationLevel) }">
                <img v-if="d.user?.avatarUrl" :src="d.user.avatarUrl" class="pub-avatar-img" />
                <span v-else>{{ d.user?.nickname?.charAt(0) || '?' }}</span>
              </div>
              <div class="pub-info">
                <span class="pub-name">{{ d.user?.nickname }}</span>
                <span class="pub-cert" :style="{ color: certColor(d.user?.certificationLevel) }">
...
<style scoped>
/* ── Base ── */
.home-page { height: 100%; overflow-y: auto; background: linear-gradient(180deg, var(--bg-primary) 0%, var(--bg-secondary) 30%, var(--bg-primary) 100%); color: var(--text-primary); }
.home-page::-webkit-scrollbar { width: 4px; }
.home-page::-webkit-scrollbar-thumb { background: var(--border-color); border-radius: 2px; }
/* ── Hero Banner ── */
.hero-banner {
...
/* ── Cards ── */
.demand-grid { display: flex; flex-direction: column; gap: 12px; padding: 0 20px 40px; }
.demand-card {
...
.preview-price {
  font-size: 17px; font-weight: 800; color: var(--accent-color); flex-shrink: 0;
}
.preview-tags {
