...
async function fetchShorts() {
  ...
  try {
    const res = await api.get('/shorts', { params: { limit: 20 } });
...
function handleVideoTap(idx: number, s: Short) {
  ...
  if (now - prev < 320) {
    // Double tap → like + cancel pending single-tap
    ...
    // Single tap → toggle play/pause (deferred)
...
<template>
  <div class="shorts-page full-width">
    ...
    <div v-else class="feed">
      <!-- Tabs -->
      <div class="feed-tabs">
        <button v-for="t in tabs" :key="t.key" class="feed-tab" :class="{ on: activeTab === t.key }" @click="activeTab = t.key">{{ t.label }}</button>
      </div>

      <EmptyState v-if="shorts.length === 0" icon="🎬" message="暂无内容" action-label="回首页" @action="router.push('/')" />
      ...
      <div v-for="(s, idx) in shorts" :key="s.id" class="case-card" :class="{ 'media-fit-contain': mediaUseContain[idx] }" :data-index="idx" @click="handleVideoTap(idx, s)">
        ...
        <img v-else-if="isImage(s.mediaUrl)" :src="s.mediaUrl" @load="onMainImageLoad($event, idx)" />
        ...
        <div class="overlay">
          ...
          <div class="action-bar">
            ...
            <button class="act-btn" @click.stop="router.push('/profile/' + s.userId)">
              <div class="act-avatar" :style="{ background: certColor((s.user as any)?.certificationLevel || 'NONE') }">
                <img v-if="s.user?.avatarUrl" :src="s.user.avatarUrl" class="act-avatar-img" />
          ...
          <div class="video-info" @click.stop="openDetail(s)">
            ...
            <div class="info-meta"><span>{{ formatTime(s.createdAt) }}</span></div>
          </div>
          ...
          <div class="panel-body">
            ...
            <div class="panel-author" @click="showDetail = false; router.push('/profile/' + selectedShort.userId)">
              <div class="pa-avatar" :style="{ background: certColor((selectedShort.user as any)?.certificationLevel || 'NONE') }">
                <img v-if="selectedShort.user?.avatarUrl" :src="selectedShort.user.avatarUrl" class="pa-avatar-img" />
            ...
            <div class="panel-grid">
              <div class="panel-grid-row"><span>发布时间</span><span>{{ formatTime(selectedShort.createdAt) }}</span></div>
...
<style scoped>
/* ── Page ── */
.shorts-page { height: 100%; overflow-y: auto; scroll-snap-type: y mandatory; background: #000; }
.shorts-page::-webkit-scrollbar { width: 0; }
...
/* ── Mute button ── */
.mute-btn { position: absolute; top: 60px; right: 14px; z-index: 3; background: none; border: none; font-size: 20px; cursor: pointer; padding: 6px; opacity: 0.6; pointer-events: auto; }
...
.act-btn.liked { color: #FE2C55; }
.act-btn.on { color: #f59e0b; }
.act-btn:disabled { opacity: 0.5; }
.act-icon { font-size: 30px; }
.act-label { font-size: 11px; font-weight: 600; }
.act-plus { width: 28px; height: 28px; border-radius: 50%; background: #FE2C55; color: #fff; display: flex; align-items: center; justify-content: center; font-size: 16px; font-weight: 700; }
...
/* ── CTA ── */
.cta-btn { pointer-events: auto; position: absolute; bottom: 20px; left: 14px; padding: 8px 20px; border-radius: 24px; border: none; background: linear-gradient(135deg, #FE2C55, #ff6b81); color: #fff; font-size: 14px; font-weight: 700; cursor: pointer; letter-spacing: 1px; box-shadow: 0 4px 16px rgba(254,44,85,0.3); }
...
.panel-sheet { width: 100%; max-height: 75vh; overflow-y: auto; background: #1a1a1f; border-radius: 20px 20px 0 0; padding: 12px 20px 28px; }
...
.pa-follow-btn { flex: 1; padding: 12px; border: none; border-radius: 10px; background: #FE2C55; color: #fff; font-size: 14px; font-weight: 700; cursor: pointer; }
