...
const Textarea = React.forwardRef<HTMLTextAreaElement, TextareaProps>(({ className, ...props }, ref) => (
  <textarea
    className={cn(
      "flex w-full rounded-md border-0 outline-none bg-transparent px-3 py-2.5 text-base text-gray-100 placeholder:text-gray-400 focus-visible:outline-none focus-visible:ring-0 disabled:cursor-not-allowed disabled:opacity-50 min-h-[44px] resize-none",
      className
    )}
    ref={ref}
    rows={1}
...
export const PromptInputBox = React.forwardRef((props: PromptInputBoxProps, ref: React.Ref<HTMLDivElement>) => {
  const { onSend = () => {}, isLoading = false, placeholder = "输入你的需求...", className, onThinkChange } = props as PromptInputBoxProps & { onThinkChange?: (think: boolean) => void };
  ...
  const handleToggleChange = (value: string) => {
    if (value === "search") {
      ...
      setShowThink((prev) => { const next = !prev; onThinkChange?.(next); return next; });
      setShowSearch(false);
    }
  };
