...
interface EmptyStateProps {
  ...
  message?: string
  actionLabel?: string
  variant?: 'default' | 'error' | 'internal'
  onAction?: () => void
...
export function EmptyState({
  ...
  if (isInternal) {
    return (
      ...
      >
        <IconComponent className="mb-4 size-10 text-text-muted opacity-50" />
        <p className="font-medium text-text-secondary">{title}</p>
        <p className="mt-1 font-mono text-xs text-text-muted">{desc}</p>
        {actionSlot ? (
