...
export function FollowList({ visible, userId, mode, onClose }: FollowListProps) {
  ...
  async function loadPage(p: number) {
    ...
    try {
      ...
      const list: any[] = data.list || data.items || data.users || data.followers || data.following || []
      // "关注"列表默认都是已关注，"粉丝"列表默认未关注
      const mapped = list.map((u: any) => ({ ...u, isFollowing: u.isFollowing ?? (mode === 'following') }))
        ...
        <>
          ...
          >
            <div className="flex items-center justify-between p-5 pb-3">
              ...
              <button onClick={onClose} className={cn('w-8 h-8 flex items-center justify-center rounded-lg', isDark ? 'bg-white/5' : 'bg-black/[0.04]')}>
            ...
            <div className="flex-1 overflow-y-auto px-4 pb-6">
              {items.map((u) => (
                <div key={u.id} className={cn('flex items-center gap-3 py-3 border-b', isDark ? 'border-white/5' : 'border-black/[0.06]')}>
                  <div
                    className={cn('w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold overflow-hidden flex-shrink-0 cursor-pointer', isDark ? 'bg-white/10' : 'bg-black/[0.06]')}
                  ...
                  {u.id !== myId && (
                    <button
                      ...
                      className={cn(
                        ...
                        u.isFollowing
                          ? cn('text-text-muted border', isDark ? 'bg-white/5 border-white/10' : 'bg-black/[0.04] border-black/[0.08]')
