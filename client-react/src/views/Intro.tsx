  ...
  return (
    <div className="fixed inset-0 overflow-y-auto bg-bg-primary text-text-primary">
      ...
      <section className="px-7 pt-20 pb-10 md:px-10 md:pt-[100px] md:pb-[60px]">
        <span className="text-xs font-bold tracking-[6px] text-text-muted block mb-5">
          NINEWOOD
        </span>
        <h1 className="text-[38px] font-black tracking-[-0.5px] leading-[1.18] mb-4 md:text-[52px]">
          每一份技能
          <br />
          都值得被看见
        </h1>
      ...
      <section className="px-5 pb-8 max-w-[680px] mx-auto">
        ...
        <div className="flex rounded-xl overflow-hidden border border-border mb-6">
          {tiers.map((tier, idx) => (
            <button
              ...
              className={cn(
                'flex-1 flex items-center justify-center gap-1.5 py-3 px-2 border-none text-[13px] font-semibold tracking-[1px] transition-all duration-[0.25s]',
                activeTier === idx
                  ? cn('bg-card', isDark ? 'text-white' : 'text-text-primary')
        ...
        <div className="rounded-[20px] overflow-hidden bg-card border border-border transition-all duration-[0.35s]">
          ...
          >
            <div
              className="relative z-[1] px-7 py-2.5 rounded-full text-base font-extrabold text-white tracking-[6px] shadow-[0_8px_32px_rgba(0,0,0,0.3)]"
          ...
          <div className="px-6 py-6">
            <span
              className="inline-block px-3 py-1 rounded text-[11px] font-bold tracking-[2px] border mb-3"
              style={{ color: t.color, borderColor: t.color }}
            >
              {t.subtitle}
            </span>
            <h2
              className="text-[26px] font-black mb-2.5"
              style={{ color: t.color }}
            >
              {t.title}
            </h2>
        ...
        <div className="relative flex justify-between items-center mt-7 px-1 h-5">
          ...
          <div
            className="absolute top-1/2 left-2 h-0.5 bg-[var(--primary-gradient)] -translate-y-1/2 rounded-sm transition-[width_0.35s]"
          ...
          {tiers.map((tier, idx) => (
            <div
              key={tier.level}
              onClick={() => setActiveTier(idx)}
              className="relative z-[1] w-7 h-7 flex items-center justify-center cursor-pointer rounded-full hover:bg-bg-tertiary"
            >
              <div
                className="rounded-full transition-all duration-[0.35s]"
                style={{
                  width: idx === activeTier ? 14 : 10,
                  height: idx === activeTier ? 14 : 10,
                  background:
                    idx <= activeTier ? tier.color : 'var(--border-color)',
                  boxShadow:
                    idx <= activeTier
                      ? `0 0 ${idx === activeTier ? 12 : 8}px ${tier.color}`
                      : 'none',
                }}
              />
            </div>
