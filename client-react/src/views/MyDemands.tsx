...
import { AcetBrutalButton } from '@/components/ui/tailwindcss-buttons-variants'
    ...
    <div className="relative z-[1] flex h-full min-h-0 w-full min-w-0 flex-col items-stretch overflow-y-auto thin-scroll bg-bg-primary">
      <div className="relative z-10 box-border flex w-full max-w-3xl shrink-0 flex-col self-center p-5">
        <div className="flex gap-2 mb-4">
          {[
            { k: 'demands' as const, l: '我的需求' },
            { k: 'applications' as const, l: '我的申请' },
          ].map((t) => (
            <button
              key={t.k}
              onClick={() => setTab(t.k)}
        ...
        {error && (
          <div className="text-center py-16">
            ...
            <AcetBrutalButton
              type="button"
              onClick={fetchData}
              className="!mx-auto !mt-3 !block !px-4 !py-2 !text-xs !normal-case"
            >
              重试
            </AcetBrutalButton>
          </div>
            ...
            ? demands.map((d: any) => (
                ...
                >
                  <div className="flex justify-between items-center">
                    ...
                    <span className="px-2 py-0.5 rounded text-[10px] font-semibold bg-bg-secondary text-text-muted">
                      {sMap[d.status] || d.status}
                    </span>
                  </div>
                  <div className="flex justify-between mt-2 text-[13px] text-text-secondary">
                    <span>{d.applicantCount || 0} 人申请</span>
                    <span>¥{d.minPrice}</span>
                  </div>
                  {d.status === 'FROZEN' && (
                    <button
                      onClick={(e) => {
            ...
            : applications.map((a: any) => (
                ...
                >
                  <div className="flex justify-between items-center">
                    ...
                    <span className="px-2 py-0.5 rounded text-[10px] font-semibold bg-bg-secondary text-text-muted">
                      {aMap[a.status] || a.status}
                    </span>
                  </div>
                  <div className="flex justify-between mt-2 text-[13px] text-text-secondary">
                    <span>¥{a.offerPrice || a.demand?.minPrice}</span>
                    <span>
